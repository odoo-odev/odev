...
from odev.utils.secrets import secret_storage, StoreSecret, DeleteSecret
...
class ShConnector(object):
    ...
    def create_session(self, login, password):
        ...
        headers = {
            'user-agent': (
                f'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 '
                f'(KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36'
            ),
        }
        ...
        authenticity_token = soup.find('input', attrs={'name': 'authenticity_token'})
        assert authenticity_token
        login_data['authenticity_token'] = authenticity_token['value']

        resp = session.post(url, data=login_data, headers=headers)

        # TODO : do something smarter
        if 'Incorrect username or password' in resp.text:
            raise ValueError('Can\'t connect to Github')
    ...
    try:
        with storage_context as session_id:
            if session_id is None:
                ...
                login = logger.ask('Github / odoo.sh login:')
                passwd = logger.password('Github / odoo.sh password:')
                save = True
            try:
