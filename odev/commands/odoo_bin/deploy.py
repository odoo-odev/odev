...
class DeployCommand(commands.LocalDatabaseCommand):
    ...
    def __init__(self, args: Namespace):
        ...
        self.login = args.login or logger.ask('Login (blank for "admin"):')
        self.password = args.password or logger.password('Password (blank for "admin"):')
    ...
    def run(self):
        ...
        if version_match:
            ...
            version = version_match[~0] if version_match else logger.ask(
        ...
        logger.info(f'''Running: {re_password.sub(f'{PASSWORD} {"*" * len(self.password or "admin")}', command)}''')
            ...
            if error_match:
                if re_import_module.search(result) or re_dependencies.search(result):
                    ...
                    raise InvalidOdooModule(
                        f'Cannot install module: either the supplied credentials '
                        f'or the imported module are invalid'
                    )

            logger.success(f'Successfully deployed module `{module_name}` to {self.url or self.database}')
